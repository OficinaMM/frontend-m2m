...
{pantallaActual === 'admin-pluses' && (
              <div style={{ textAlign: 'left' }}>
                <h2 style={{ color: '#b27d14', marginTop: 0, fontSize: '20px', borderBottom: '2px solid #eee', paddingBottom: '10px' }}>⭐ Pluses y Ajustes</h2>
                ...
                <form onSubmit={manejarGuardarPlus} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '25px', background: '#fcfcfc', padding: '15px', borderRadius: '8px', border: '1px solid #e0e0e0' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    {/* Se ha eliminado o ampliado cualquier límite para aceptar números negativos */}
                    <input 
                      type="number" 
                      step="0.01" 
                      placeholder="Importe (€) (Ej: -50 o 50)" 
                      value={montoPlus} 
                  ...
                  <input type="text" placeholder="Concepto (Ej: Descuento material / Plus mensual)..." value={conceptoPlus} onChange={(e) => setConceptoPlus(e.target.value)} style={{ padding: '8px', borderRadius: '6px', border: '1px solid #ccc', fontSize: '14px', boxSizing: 'border-box' }} />
                ...
                <h3 style={{ fontSize: '15px', color: '#333', marginBottom: '10px' }}>Historial de Pluses y Ajustes</h3>
                {historialPluses.length === 0 ? (
                  <p style={{ textAlign: 'center', color: '#666', padding: '15px' }}>No hay registros.</p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '35vh', overflowY: 'auto', paddingRight: '4px' }}>
                    {historialPluses.map((plus) => {
                      ...
                      return (
                        <div key={plus.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', background: '#fff', border: '1px solid #eee', borderRadius: '6px', fontSize: '13px' }}>
                          <div>
                            {/* Color dinámico: rojo para negativos/descuentos, dorado para positivos */}
                            <div style={{ fontWeight: 'bold', color: esNegativo ? '#d32f2f' : '#b27d14' }}>
                              {esNegativo ? '🔻' : '⭐'} {Number(plus.importe).toFixed(2)} €
                            </div>
