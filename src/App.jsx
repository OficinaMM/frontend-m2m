...
{pantallaActual === 'admin-pluses' && (
  <div style={{ textAlign: 'left' }}>
    <h2 style={{ color: '#b27d14', marginTop: 0, fontSize: '20px', borderBottom: '2px solid #eee', paddingBottom: '10px' }}>⭐ Plus de Productividad y Ajustes</h2>
    ...
    <form onSubmit={manejarGuardarPlus} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '25px', background: '#fcfcfc', padding: '15px', borderRadius: '8px', border: '1px solid #e0e0e0' }}>
      <h3 style={{ margin: '0 0 5px 0', fontSize: '15px', color: '#043424' }}>Asignar Plus o Ajuste a Empleado</h3>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        <div style={{ flex: 1 }}>
          <label style={{ fontSize: '11px', color: '#666', display: 'block', marginBottom: '3px' }}>Importe (€) <span style={{ fontSize: '10px', color: '#888' }}>(Usa el signo - para restar/deuda)</span>:</label>
          <input type="number" step="0.01" placeholder="Ej: 50 o -30" value={montoPlus} onChange={(e) => setMontoPlus(e.target.value)} required style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #ccc', fontSize: '14px', boxSizing: 'border-box' }} />
        </div>
        <div style={{ flex: 1 }}>
          <label style={{ fontSize: '11px', color: '#666', display: 'block', marginBottom: '3px' }}>Fecha:</label>
          <input type="date" value={fechaPlus} onChange={(e) => setFechaPlus(e.target.value)} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #ccc', fontSize: '14px', boxSizing: 'border-box' }} />
        </div>
      </div>

      <input type="text" placeholder="Concepto (Ej: Productividad o Ajuste anticipo/deuda)..." value={conceptoPlus} onChange={(e) => setConceptoPlus(e.target.value)} required style={{ padding: '8px', borderRadius: '6px', border: '1px solid #ccc', fontSize: '14px', boxSizing: 'border-box' }} />

      <button type="submit" style={{ padding: '12px', background: '#b27d14', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>💾 Guardar Ajuste / Plus</button>
    </form>
    ...
    {historialPluses.length === 0 ? (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '35vh', overflowY: 'auto', paddingRight: '4px' }}>
        {historialPluses.map((plus) => {
          const esNegativo = Number(plus.importe) < 0;
          return (
            <div key={plus.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', background: '#fff', border: '1px solid #eee', borderRadius: '6px', fontSize: '13px' }}>
              <div>
                <div style={{ fontWeight: 'bold', color: esNegativo ? '#d32f2f' : '#b27d14' }}>
                  ⭐ {esNegativo ? `${Number(plus.importe).toFixed(2)} €` : `+${Number(plus.importe).toFixed(2)} €`}
                </div>
              ...
              <button 
                onClick={async () => {
                  if (window.confirm("¿Seguro que deseas eliminar este registro de productividad?")) {
                    try {
                      ...
                      const { data: datosActualizados } = await supabase.from('PLUS PRODUCTIVIDAD').select('*');
